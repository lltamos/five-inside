         ...
         <div class="proving">
             ...
             <div class="form_1">
                 <ul>
                     ...
                     <c:if test="${bdOrderList.size() <= 0 }">
                    	<%@include file="/common/nodata.jsp"%>
	                 </c:if>
	                 <c:if test="${bdOrderList.size() > 0 }">
	                     <c:forEach items="${bdOrderList}" var="bdOrder" varStatus="status">
		                     <li class="bor_gray">
		                         <span class="wid_1">${bdOrder.order_num}</span>
		                         <span class="wid_2"><fmt:formatDate value="${bdOrder.created_time}" pattern="yyyy-MM-dd HH:mm:ss" type="date"/></span>
		                         <span class="wid_3">${bdOrder.total_money}</span>
		                         <span class="wid_4">${bdOrder.name}</span>
		                         <span class="wid_5">${bdOrder.userid}</span>
		                         <span class="wid_6">
		                         <c:if test="${bdOrder.states == 1 }">
									<i>订单已支付</i> 
								 </c:if>
		                   		 <c:if test="${bdOrder.states == 2 }">
									<i>订单已配送</i> 
								 </c:if>
		                    	 <c:if test="${bdOrder.states == 3 }">
									<i>订单已完成</i> 
								 </c:if>
		                    	 <c:if test="${bdOrder.states == 4 }">
									<i>订单已撤销</i> 
								 </c:if>
		                         </span>
		                         <span class="wid_7">
		                             <a href="javascript:;" style="color: #0c48d6;" onclick="open_1('${bdOrder.order_num}')">详情</a>
		                         </span>
		                     </li>
	                     </c:forEach>
	                     </div>
	    ...
	    $(function(){
	    	mypager.generPageHtml({
	    		...
	    		click:function(currPage){
	    			$("#aftersales_index").load("${ctx }/admin/after/aftersales/shoporder/findAllBDOrder?mobile=${param.mobile}&rows=10&page="+currPage);
	    		}
	    	});
	    });
	    function findAll(){
	   		var mobile = $("#mobile").val();
	   		pageGo("${cxt }/admin/after/aftersales/shoporder/findAllBDOrder?mobile="+mobile);
	    }
