...
<body>
    ...
    <div class="message_right">
        <div class="proving">
            <div class="date">
                <ul>
                    <li><span>开始日期: </span><input type="text"  id="begin_time" value="${param.begin_time }" /></li>
                    <li><span>结束日期: </span><input type="text"  id="end_time" value="${param.end_time }" /></li>
                    <li><span>产品名称: </span><input type="text"  id="good_name" placeholder="产品名称" value="${param.good_name }" /></li>
                    <li>
                        <div class="btn_1">
            ...
            <div class="form_1">
                <ul>
                    ...
                    <c:if test="${kgSaveSellStatementList.size() <= 0 }">
                    	<%@include file="/common/nodata.jsp"%>
                    </c:if>
                    <c:if test="${kgSaveSellStatementList.size() > 0 }">
	                    <c:forEach items="${kgSaveSellStatementList}" var="saveSellStatement" varStatus="status">
		                    <li class="bor_gray">
		...
		function getPage(){
			...
			mypager.generPageHtml({
				...
				click:function(currPage){
					$("#lib-content").load("${cxt }/admin/after/libraryTube/savesellstatement/findStatementAll?begin_time="+encodeURIComponent(begin_time)+"&end_time="+encodeURIComponent(end_time)+"&good_name="+encodeURIComponent(good_name)+"&rows=10&page="+currPage);
				}
			});
		}
		function findAll(){
			...
			var end_time = $("#end_time").val();
			var good_name = $("#good_name").val();
			pageGo("${cxt }/admin/after/libraryTube/savesellstatement/findStatementAll?begin_time="+encodeURIComponent(begin_time)+"&end_time="+encodeURIComponent(end_time)+"&good_name="+encodeURIComponent(good_name));
		}
		function deriveCxStatement(){
			var begin_time = $("#begin_time").val();
			var end_time = $("#end_time").val();
			var good_name = $("#good_name").val();
			window.location.href="${cxt }/admin/after/libraryTube/savesellstatement/deriveCxStatement?begin_time="+encodeURIComponent(begin_time)+"&end_time="+encodeURIComponent(end_time)+"&good_name="+encodeURIComponent(good_name);
	    }
		function deriveKcStatement(){
			window.location.href="${cxt }/admin/after/libraryTube/savesellstatement/deriveKcStatement";
	    }
		$("#begin_time").jeDate({
		    isinitVal:false,
		    festival:true,
