...
$(function(){
	mypager.generPageHtml({
		...
		click:function(currPage){
			$("#chinamerchants_index").load("${ctx }/admin/after/provider/toAssociatedPage?page="+currPage+"&goodName=${param.goodName}&barCode="+escape("${param.barCode}")+"&proName=${proName }&providerId=${providerId}");
		}
	});
...
function queryInfo() {
	...
	pageGo("${ctx }/admin/after/provider/toAssociatedPage?goodName="+encodeURIComponent(goodName)+"&barCode="+escape(barCode)+"&proName=${proName }&providerId=${providerId}");
...
function associatedGoods(){
	...
	$.ajax({
		  type: 'POST',
		  url: "${ctx }/admin/after/provider/associatedAdd?goodsid="+goodsid+"&providerId=${providerId}",
		  dataType:"json",
		  success : function (data){
			  alert(data.msg);
			  pageGo("${ctx }/admin/after/provider/toAssociatedPage?goodName=${param.goodName}&barCode=${param.barCode}&proName=${proName }");
		  }
	  });
}
            ...
            <div class="form_1">
                <ul>
                    ...
                    <c:forEach items="${associatedList }" var="goods" varStatus="status">
                    	<li class="bor_gray">
	                        <span class="wid_1"><input name="checkgood" id="checkid${goods.id }" onclick="checkedGood('${goods.id }')" <%-- onclick="saveCookie('${goods.id }')" --%> type="checkbox" value="${goods.id }"/></span>
	                        ...
	                        <span class="wid_5">&nbsp;${goods.created_time }</span>
	                    </li>
                    </c:forEach>
                    </div>
		            <div class="paging">
		            ...
		            <div class="btn_93">
		                <a onclick="associatedGoods()" class="btn_13" href="javascript:;">提交关联</a>
		            </div>
		            </c:if>
                </ul>
        </div>
