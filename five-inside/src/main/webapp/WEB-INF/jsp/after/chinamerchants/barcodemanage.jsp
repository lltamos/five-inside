...
$(function(){
	mypager.generPageHtml({
		...
		click:function(currPage){
			$("#chinamerchants_index").load("${ctx }/admin/after/good/toGoodPageList?page="+currPage+"&goodName=${param.goodName}&barCode="+escape("${param.barCode}"));
		}
	});
});
...
function queryInfo() {
	...
	pageGo("${ctx }/admin/after/good/toGoodPageList?goodName="+encodeURIComponent(goodName)+"&barCode="+escape(barCode));
...
function removeBarCode(goodId,bar_code){
	if(bar_code == null || bar_code == ''){
		...
		if(window.confirm("您确定要移除条码吗？")){
			$.ajax({
				url : "${ctx}/admin/after/good/removeBarCode?goodId="+goodId,
				type : 'POST',
				success : function(result) {
					var appResult = eval("(" + result + ")");
					var message = appResult.msg;
					alert(message);
					$("#chinamerchants_index").load("${ctx }/admin/after/good/toGoodPageList?page=${param.page}&goodName=${param.goodName}&barCode="+escape("${param.barCode}"));
				}
			})
		}
	}
...
function bindingBarCode(){
	...
	$.ajax({
		url : "${ctx}/admin/after/good/bindingBarCode?goodId="+goodsids+"&barCode="+barCode,
		type : 'POST',
		success : function(result) {
			var appResult = eval("(" + result + ")");
			if(appResult.code==0){
			...
			pageGo("${ctx }/admin/after/good/toGoodPageList?goodName=${param.goodName}&barCode=${param.barCode}");
		}
	})
}
</script>
