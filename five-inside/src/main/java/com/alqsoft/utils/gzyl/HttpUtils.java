...
public class HttpUtils {
    ...
    public static String execute(String url, String jsonParam,
        ...
        try {
            ...
            logger.info(String.format("[url=%s][req=%s]", url, jsonParam));
            HttpResponse resp = httpClient.execute(method);
            int statusCode = resp.getStatusLine().getStatusCode();
            respString = EntityUtils.toString(resp.getEntity());
            logger.info(String.format("[statusCode=%s][resp=%s]", resp.getStatusLine().getStatusCode(), respString));
            // 通讯成功
            ...
            respJson.put(SystemRetField.RET_CODE.toString(), SystemRetCode.EXCPETION.toString());
